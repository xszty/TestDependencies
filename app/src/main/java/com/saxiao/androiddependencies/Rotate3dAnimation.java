...
public class Rotate3dAnimation extends Animation {
    ...
    Description parssds(TypedValue value){
		...
		if(value == null){
			...
			if(value.type == TypedValue.TYPE_FRACTION){
				i.value = TypedValue.complexToFloat(value.data);
				return i;
			}else if(value.type == TypedValue.TYPE_FLOAT){
				i.value = value.getFloat();
				return i;
			}else{
				i.type = ABSOLUTE;
				i.value = value.data;
    ...
    private void initial(int width,int height,int parentWidth,int parentHight){
    	mCamera = new Camera();
    	mPivotX = resolveSize(mPivotXType,mPivotXValue,width,parentWidth);
    	mPivotY = resolveSize(mPivotYType,mPivotYValue,width,parentHight);
    }

	private void getInnal(int width,int height){
		mCamera = new Camera();
		mPivotX = resolveSize(mPivotXType,mPivotXValue,width,height);
		mPivotY = resolveSize(mPivotYType,mPivotYValue,width,height);
	}

	private void getInnallds(){
		mCamera = new Camera();
	}
